...
var sUrl = "http://localhost:8000/slip/";
...
function setDataFields() {
    ...
    var fields = [{
        ...
        calculateCallback: function calculateCallback(dataRow, fieldName, fieldNames, values) {
            var no_acct = values[fieldNames.indexOf("no_acct")];

            if (isNaN(no_acct) || isNaN(no_acct)) return undefined;else return padLeft(no_acct, 5, '0');
        }
    }, {
        fieldName: "da_date",
        dataType: RealGridJS.DataType.NUMBER
...
function setGridCallbackFunc() {
    ...
    gridView.onCellEdited = function (grid, itemIndex, dataRow, field) {
        ...
        if (focusCell.fieldName == "mn_bungae1" || focusCell.fieldName == "mn_bungae2") {
            ...
            var colNames = gridView.getColumnNames(true);
            var nextColName = colNames[0];
...
function setProviderCallbackFunc() {
    ...
    dataProvider.onRowUpdated = function (provider, row) {
        var sqActtax2 = findFieldValue("sq_acttax2", row);
...
function findFieldValue(pfieldName, rowIndex) {
    ...
    if (value != undefined && value.toString().length > 0) return value;else {
        return null;
    }
}
...
function getdaDate(pMon, pDay, itemIndex) {
    ...
    if (value != null && value.length == 6 && pDay != "") {
        if (pMon == "") rtnVal = value.substring(0, 6) + padLeft(pDay, 2, '0');else rtnVal = value.substring(0, 4) + padLeft(pMon, 2, '0') + padLeft(pDay, 2, '0');
    }

    if (rtnVal != undefined && rtnVal.length == 8) return rtnVal;else return rtnVal;
}

function getmnBungae(ptyGubn, itemIndex) {

    var mnBungae = void 0;
    if (ptyGubn == 2 || ptyGubn == 4) mnBungae = findFieldValue('mn_bungae2', itemIndex);else mnBungae = findFieldValue('mn_bungae1', itemIndex);

    if (mnBungae != undefined && mnBungae > 0) return mnBungae;else return 0;
}

function CheckInputValue(pCheckParam) {
    var rtnFlag = true;
    for (var key in pCheckParam) {
        switch (key) {
            case "da_date":
                {
...
function callInsertAPI(grid, provider, itemIndex, datarow) {
    ...
    var daDate = getdaDate("", findFieldValue('day', datarow), datarow);
...
function callDeleteAPI() {
    ...
    for (var item in rows) {
        var sqActtax2 = findFieldValue("sq_acttax2", rows[item]);
